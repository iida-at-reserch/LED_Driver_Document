...
外観検査向けLEDドライバはカメラの露光タイミングと同期して、整定後の明るさに高い繰り返し再現性があることや低遅延かつ低ジッタで動作することが要求される。
...
しかし、露光中に一定周期で点灯と消灯と繰り返すため、ローリングシャッターのカメラでは画像内で明るさムラ（縞模様）が発生してしまう他、
...
\section{現行LEDドライバーを理解しよう}

本章では、現行のLEDドライブ回路の動作機構を理解するとともに、現状起こっている発振やその対策、今後改善すべき課題を明らかにする。
\subsection{なぜLEDに流れる電流をコントロールできるのか？}
DY-CY-140,DR-X-147のLEDドライブ回路を図に示す。
両者は細かい点で違いは見られるが、共通のトポロジーを持つ回路である。
この回路は破線の左右で前段と後段で機能が分けられる。
簡単に解説すると、前段回路は後段回路で目標電流を駆動するのに必要な電圧信号を生成し、
後段回路はこれを目標値として、フィードバック制御によりLEDに向けて電流を駆動する。
直流解析でさらに詳しく動作機構を解説する。

DR-X-147の回路をもとに説明する。
図中の$DAC  OUT0$が目標値入力、$VDD1$〜$VDD3$が電源、$LED_CH0$が電流出力である。
$DAC{OUT0}$から目標値を0Vから$V_{ref}$に変化させた場合の各箇所の電圧および電流がどのように変化するか考える。
まず、前段回路のオペアンプの非反転入力端子の電圧が上昇することで、オペアンプの出力電圧が上昇し、トランジスタQ4のコレクタ電流が増加する。
定常状態では、負帰還制御によってオペアンプU3Aの非反転入力端子と反転入力端子は同電位となるため、$R_{39}$の両端電圧は$V_{ref}$となる。
よって、定常状態におけるトランジスタ$Q4$のコレクタ電位$V_{C}$は次のように示せる。
\begin{equation}
    V_{C} = V_{DD} - \frac{V_{ref}}{R_{39}} \cdot (R_{37} + R_{38})
\end{equation}

$V_{C}$は後段回路のオペアンプの非反転入力端子に接続されており、$V_{C}$が低下すると、反転入力端子の電位よりも非反転入力端子の電位の方が低くなるため、
オペアンプの出力電圧が低下する。そのため、その先に接続されたMOSFETのゲート電圧が低下することで、ゲート-ソース間電圧が大きくなり、電流が駆動される。
電流が流れると、ソースの電位は低下するため、フィードバックされているオペアンプの反転入力端子の電位も下がり、結果として非反転入力端子と同じ電位まで下がったところで、電流は整定する。
このとき、センス抵抗→MOSFETを通り、LEDに向けて出力される電流を$I_{LED}$とおくと、反転入力端子の電位は次のように示せる。
\begin{equation}
    V_{-} = \frac{V_{DD} - R_{64}I_{LED}}{R_{46}+R_{49}} \cdot R_{46}
\end{equation}

$V_{-} = V_{C}$より、式を整理すると、$I_{LED}$は次のようになる。

\begin{equation}
    I_{LED} = \frac{(R_{37}+R_{38})(R_{46}+R_{49})}{R_{39}R_{46}R_{64}}V_{ref} - \frac{R_{49}}{R_{46}R_{64}}V_{DD}
\end{equation}


$R_{37}=560Ω$、$R_{38}=470Ω$、$R_{39}=2kΩ$、$R_{46}=1MΩ$、$R_{49}=1kΩ$、$R_{64}=2Ω$より、

\begin{gather}
    \begin{aligned}
        I_{LED} &= 0.2578V_{ref} - \frac{V_{DD}}{2000} \\
        &\approx 0.2578V_{ref}
    \end{aligned}
\end{gather}

式(4)より、$V_{ref}$と電流値は比例関係にあり、デジタル-アナログコンバータ（DAC）などで$V_{ref}$を制御することで、任意の電流をLEDに流すことができることが分かる。
$V_{DD}$の項の役割については、後ほど説明する。

%動作機構の解説
 %電流吐き出し型の定電流回路が採用される
 %直流解析
 %ラプラス変換　伝達関数モデル
\subsection{なぜ狙った電流値に収束しないのか？}
前節では、現行LEDドライブ回路の各構成要素がどのように作用し、目標値に整定するか解説した。
しかし、実際に基板で動作させてみると、目標値に整定せず、電流が持続的に振動あるいは発散してしまう場合がある。
本節では、実験結果を交えながら、このとき発生する「発振」という現象について解説する。
\subsubsection{発振とは？}
　負帰還増幅回路における発振とは、本来システムを安定させるための負帰還が、特定の高周波帯域において意図せず正帰還へと反転してしまい、
入力信号がなくても回路自身が連続して波形を出力し続けてしまう現象を指す。
注意点として、ここ示す「発振」とは、あくまで有限時間内に整定せず、持続的にフィードバックループを循環し続ける現象であるため、
リンギングといった立ち上がり直後に一定時間の振動は発生するものの、時間の経過とともに減衰し、最終的に一定値に収束するのものについて、ここでは「発振」とは扱わない。
ここで、実際にDR-CY-140で発振が発生している様子を図　に示す。電流出力端子からは〇mの照明ケーブルを介して照明に接続されている。
このとき、目標値は　としているが、　mAで整定せず激しく振動していることが分かる。

%発振せずに収束してる画像と比較するか？

\subsubsection{なぜ発振が起こるのか？}
　本項では、前項で示した発振という現象がどのようなメカニズムで発生するか解説する。
図　に示すコントローラー$C(s)$、制御対象$G(s)$、帰還回路$H(s)$のフィードバックシステムを用いて考える。
%図　フィードバックシステム
このシステムは入力した目標値と出力からの帰還信号の差分をとった後、コントローラー$C(s)$で増幅し、制御対象$G(s)$に与える。そして、出力を帰還回路を通してフィードバックしている。
最終的には目標値と帰還信号の差分が0となるように制御される。

通常の安定したシステムでは、加え合わせ点での減算によって設計者の意図する負帰還として機能する。
しかし、制御対象や要素の特性によって、特定の周波数信号が一巡する間に位相が大きく遅れた場合、問題が生じる。
帰還時の位相遅れが180°に達し、加え合わせ点での減算（-180°回転）すると、位相が360°回転した
状態となる。これは、入力と同位相の信号が足し合わされることを意味し、正帰還として働いてしまうのである。
このとき、ループ内でのゲイン増幅率が1未満であれば、信号は循環するたびに減衰するため、発振は起こらない。
しかし、ゲインが1以上の場合は信号が減衰せず、持続あるいは発散していくため、結果として発振が引き起こされる。
以上が発振が引き起こされる感覚的なイメージであるが、
実際にシステムが発振に至る条件を定量的に示すため、伝達関数を用いて解説する。
このシステムの閉ループ伝達関数$\frac{Y(s)}{X(s)}$は次のように書ける。
\begin{equation}
    \frac{Y(s)}{X(s)} = \frac{C(s)G(s)}{1+C(s)G(s)H(s)}
\end{equation}
このシステムが発振する条件は、分母$1+C(s)G(s)H(s) = 0$なり発散する場合である。
すなわち、一巡伝達関数$C(s)G(s)H(s)$のゲインと位相遅れが次の条件を満たす場合である。
\begin{gather*}
    \begin{aligned}
    ゲイン：\left|C(s)G(s)H(s)\right| = 1 \\
    位相：C(s)G(s)H(s) \angle -180^\circ        
    \end{aligned}
\end{gather*}
すなわち、システムの安定性を確保するためには、一巡伝達関数において、
ゲイン交差周波数（ゲインが0dBとなる周波数）での位相遅れが180°未満であり、
位相交差周波数（位相遅れが180°となる周波数）でのゲインが0dB未満である必要がある。
この基準に基づいて、システムの安定度を定量的に評価する指標として、
ゲイン交差周波数における180°までの位相角の余裕を示す「位相余裕」と、
位相交差周波数における0dBまでのゲインの余裕を示す「ゲイン余裕」が広く用いられる。

\subsection{狙った電流値に収束させるには？それによって生じる問題は何か？}
%課題・改善点
先述した通り、システムを安定化させるには、回路特性を調整して十分な安定余裕（位相余裕・ゲイン余裕）を確保する必要がある。
とりわけ、オペアンプを用いた負帰還増幅回路などにおいて、位相やゲインを調整して安定化を図る手法を「位相補償」と呼ぶ。
現行の回路においても、この位相補償の仕組み自体は既に組み込まれている（その定数設定が十分であるかは別途検証が必要ではあるが）。
具体的には、図〇のキャパシタ（C〇）がその役割を負っている。
このキャパシタは高周波帯域におけるオペアンプ出力の帰還量を増加させ、
ループゲインを減衰させることで、先述したゲイン余裕および位相余裕を確保している。
以下では、この補償メカニズムについて伝達関数を用いて詳しく解説する。
